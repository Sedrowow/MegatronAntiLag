...
local rules = {
    ...
    "3) Vehicle that cause many warning messages as well as laggy vehicles are restricted!",
...
function onPlayerJoin(steam_id, name, user_id, admin, auth)
    ...
    local ui_id = user_id * 100 + player_ui_ids[user_id]
...
function onCustomCommand(full_message, peer_id, is_admin, is_auth, command, one, two, three, four, five)
    ...
    if (command == "?rules") then
        ...
        showPopup(peer_id, ui_id, "To get auth, type ?auth "..peer_id.." 'accept'")
    end
